...
import { Bell, Search, Plus, BookOpen, FileText, MessageSquare, ChevronDown, LogOut, User } from 'lucide-react';
...
export default function TeacherPortal() {
  ...
  const students = [
    { id: 1, initials: 'EJ', name: 'Emma Johnson', lessons: 75, engagement: 85 },
    { id: 2, initials: 'LS', name: 'Liam Smith', lessons: 92, engagement: 68 },
    { id: 3, initials: 'OD', name: 'Olivia Davis', lessons: 45, engagement: 93 },
    { id: 4, initials: 'NW', name: 'Noah Wilson', lessons: 88, engagement: 79 },
    { id: 5, initials: 'SM', name: 'Sophia Martinez', lessons: 65, engagement: 82 },
    { id: 6, initials: 'LB', name: 'Lucas Brown', lessons: 78, engagement: 70 },
  ];
  
  const getFilteredStudents = () => {
    if (filterOption === 'Needs Attention') {
      return students.filter(student => student.lessons < 60 || student.engagement < 70);
    } else if (filterOption === 'High Progress') {
      return students.filter(student => student.lessons > 80 && student.engagement > 80);
    }
    return students;
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f5f3e8]">
      {/* Use the NavBar component and pass the active page */}
      <NavBar activePage="Teacher Portal" />
      
      <div className="flex flex-1 overflow-hidden">
        {/* Sidebar */}
        <div className={`bg-[#5c6d63] text-white w-64 flex-shrink-0 ${mobileMenuOpen ? 'block' : 'hidden'} md:block`}>
          ...
          <nav className="px-4">
            <button 
              ...
              className={`w-full flex items-center p-3 mb-2 rounded-lg ${activeNav === 'dashboard' ? 'bg-[#2d3e4e]' : 'hover:bg-[#2d3e4e]'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center p-3 mb-2 rounded-lg ${activeNav === 'add-story' ? 'bg-[#2d3e4e]' : 'hover:bg-[#2d3e4e]'}`}
            >
              <FileText className="mr-3" size={20} />
              <span>Add Story</span>
            </button>
            ...
            <button 
              ...
              className={`w-full flex items-center p-3 mb-2 rounded-lg ${activeNav === 'learning-path' ? 'bg-[#2d3e4e]' : 'hover:bg-[#2d3e4e]'}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center p-3 mb-2 rounded-lg ${activeNav === 'forum' ? 'bg-[#2d3e4e]' : 'hover:bg-[#2d3e4e]'}`}
            >
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <header className="bg-white shadow-sm flex items-center justify-between p-4">
            <div className="flex items-center">
              ...
              <div className="text-xl font-medium">{activePage.charAt(0).toUpperCase() + activePage.slice(1).replace('-', ' ')}</div>
            </div>
                ...
                {notificationsOpen && (
                  <div className="absolute right-0 mt-2 w-80 bg-white rounded-lg shadow-lg p-4 z-10">
                    ...
                    <div className="border-b pb-2 mb-2">
                      <p className="text-sm">Olivia Davis completed a new assignment</p>
                      <p className="text-xs text-gray-500">10 minutes ago</p>
                    </div>
                    <div>
                      <p className="text-sm">Class report for week 12 is available</p>
          ...
          <main className="flex-1 overflow-y-auto bg-[#f5f3e8] p-6">
            {activePage === 'dashboard' && (
              <div>
                <div className="mb-8">
                  <h1 className="text-3xl font-medium text-[#2d3e4e] mb-2">Welcome back, Jane!</h1>
                  <p className="text-[#5c6d63]">Here's what's happening with your classroom today.</p>
                </div>
                
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-2xl font-medium text-[#2d3e4e]">Class 5B Progress</h2>
                    ...
                    <div className="relative">
                      ...
                      >
                        {filterOption} <ChevronDown size={16} className="ml-2" />
                      </button>
                      ...
                      {dropdownOpen && (
                        <div className="absolute right-0 mt-1 w-40 bg-white rounded-lg shadow-lg z-10">
                          <button 
                            ...
                            onClick={() => {setFilterOption('All Students'); setDropdownOpen(false);}}
                          >
                            All Students
                          </button>
                          <button 
                            className="w-full text-left px-4 py-2 hover:bg-[#f5f3e8]"
                            onClick={() => {setFilterOption('Needs Attention'); setDropdownOpen(false);}}
                          >
                            Needs Attention
                          </button>
                          <button 
                            className="w-full text-left px-4 py-2 hover:bg-[#f5f3e8] rounded-b-lg"
                            onClick={() => {setFilterOption('High Progress'); setDropdownOpen(false);}}
                          >
                            High Progress
                          </button>
                        </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {getFilteredStudents().map(student => (
                      <div key={student.id} className="bg-white rounded-lg shadow p-6">
                        ...
                        <h3 className="text-center text-lg font-medium mb-4">{student.name}</h3>
            ...
            {activePage === 'add-story' && (
              <div>
                <h1 className="text-2xl font-medium mb-6 text-[#2d3e4e]">Add Story</h1>
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="mb-4">
                    <label className="block text-sm font-medium mb-1">Story Title</label>
                    <input type="text" className="w-full border rounded p-2" placeholder="Enter story title" />
                  </div>
                  ...
                  <div className="mb-4">
                    <label className="block text-sm font-medium mb-1">Reading Level</label>
                  ...
                  <div className="flex justify-end">
                    <button className="bg-[#e26d5a] text-white px-4 py-2 rounded">Save Story</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {activePage === 'learning-path' && (
              <div>
                <h1 className="text-2xl font-medium mb-6 text-[#2d3e4e]">Create Learning Path</h1>
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="mb-4">
                    <label className="block text-sm font-medium mb-1">Path Title</label>
                    <input type="text" className="w-full border rounded p-2" placeholder="Enter path title" />
                  </div>
                  ...
                  <div className="mb-4">
                    <div className="flex justify-between items-center mb-2">
                      <label className="block text-sm font-medium">Learning Modules</label>
                    ...
                    <div className="border rounded p-4">
                      <div className="flex justify-between mb-2">
                        <h3 className="font-medium">Module 2: Advanced Topics</h3>
                        <div className="text-gray-500">3 resources</div>
                      </div>
                      <p className="text-sm text-gray-600">In-depth exploration of key concepts</p>
                    </div>
                  </div>
                  
                  <div className="flex justify-end">
                    <button className="bg-[#e26d5a] text-white px-4 py-2 rounded">Create Path</button>
                  </div>
                </div>
              </div>
            )}
            ...
            {activePage === 'forum' && (
              <div>
                <h1 className="text-2xl font-medium mb-6 text-[#2d3e4e]">Community Forum</h1>
                <div className="bg-white rounded-lg shadow mb-6">
                  ...
                  <div className="p-6">
                    ...
                    ></textarea>
                    <div className="flex justify-end">
                      <button className="bg-[#e26d5a] text-white px-4 py-2 rounded">Post</button>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow mb-4">
                  ...
                  <div className="p-4">
                    <p className="mb-4">Has anyone tried the new storytelling activity with their class? I'd love to hear some feedback before I implement it next week.</p>
